...
def compute_score(info_dic, maps_dic, weights = [1,1,1,1]):
    ...
    for i, transport in enumerate(sorted(info_dic.keys())):
        ...
        if not dist_dic:
            for k in ["emission", "calories", "price", "distance", "duration", "total_weighted_score",
            "total_weighted_score_col", "duration_score", "emission_score", "price_score", "calories_score",
            "duration_score", "emission_score", "price_score", "calories_score"]:
                out_dic[transport][k] = 0
            out_dic[transport]["coordinates"]=[]
            continue

        total_duration = round(sum(dur_dic.values()),2) # duration is sum of all durations
        ...
        value_arr.append([total_duration, total_emissions, total_price, total_calories])

        out_dic[transport] = {"emission": total_emissions, "calories":total_calories, "price":total_price, "distance":total_distance, "duration":total_duration}
...
if __name__=="__main__":
    ...
    maps_dic = get_directions((47.3857, 8.5668), (47.3649, 8.5469)) #(47.3495, 8.4920)) #
    # print(maps_dic)
