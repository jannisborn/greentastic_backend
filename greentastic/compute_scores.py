...
def compute_score(
    ...
    # define base_price_once_transits to avoid counting the base price multiple
    # times when bus comes directly after tram
    base_price_once_transits = [
        'tram', 'bus', 'trolleybus', 'commuter_train', 'subway', 'metro_rail'
    ]
    # previous step --> if prev_step=bus and step_key = tramthen don't take
    # base price again
    prev_step = None
    # Catch division by 0
    if np.sum(weights) == 0:
    ...
    for i, transport in enumerate(
    ...
    ):  # walk, escooter, ebike # sorted(info_dic.keys())
        # print('\n TRANSPORT:', transport)
        maps_key = info_dic[transport][
            'maps_key']  # current corresponding key, eg transit

        # dist_dic is {tram: 5, walking:0.5, bus: 3} = maps_dic['transit']
        # wenn eigentlich transit ist aber dann weg aufgeteilt
        dist_dic = maps_dic.get(maps_key, {}).get('distance', {})
        dur_dic = maps_dic.get(maps_key, {}).get('duration', {})

        # happens if one option does not exist, e.g. thisroute is not possible
        # by car
        if not dist_dic:
            continue
        ...
        calories = []
        toxicity = []
        for j, step_key in enumerate(
            sorted(dist_dic.keys())
        ):  # step_key = one of transit, walking, cycling, driving
            # for each part of the way, compute the absolute amount of calories
            # burnt,
            # the price and the absolute amount of emissions
            # step_key is now for example 'tram', maps_key is 'transit'
            ...
            # To avoid counting the base price twice if first bus than tram
                ...
                'priceKm' in infos and prev_step in base_price_once_transits
                and step_key in base_price_once_transits
            ):
                p = infos['priceKm'] * d
            elif 'priceKm' in infos:
                p = infos['base_price'] + infos[
                    'priceKm'
                ] * d  # public transport: base price + price per km
        ...
        out_dic[transport]['coordinates'] = maps_dic.get(maps_key, {}).get(
            'coordinates', []
        )
